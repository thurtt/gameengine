...
line_of_sight::line_of_sight( float fov, float dov ) :
_fov(fov),
_dov(dov)
{
}
...
void line_of_sight::draw( float x, float y, float angle )
{
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslatef(x, y, 0.0);	// <- the translation used was world x,y, so it was rotating around screen 0,0
	...
	glVertex2f( x - ( _fov / 2 ), y );
	glVertex2f( x + ( _fov / 2 ), y );
	
	// top side
	glVertex2f( x + ( _fov / 2 ), y );
	glVertex2f( x + ( _fov / 2 ), y + _dov );
